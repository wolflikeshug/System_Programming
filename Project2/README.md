...
| -l length    | only words of at least the indicated length should be added to the trove-file. *length* must be a positive integer, for example: *-l 6*.  If the -l option is not provided, the default length is **4**. (please notice 1. the word length is not allowed greater than 2147483647 2. if you add this argument, the search and remove function will be disabled) |
